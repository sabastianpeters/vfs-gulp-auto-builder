...
class GoogleDrive {
    ...
    async uploadFile ({ localPath, mimeType, parentFolderId = 'root', targetFileId = undefined }){
        ...
        if(targetFileId != undefined){
            ...
            driveParams.requestBody.parents = undefined;
            driveParams.fileId = targetFileId;

            res = await drive.files.update(driveParams, gaxiosParams);
        } else {
            res = await drive.files.create(driveParams, gaxiosParams);
        }
